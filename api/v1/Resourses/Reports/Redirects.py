...
class RedirectsReport(Resource):
    def __init__(self):
        ...
        self.reqparse.add_argument('from_date', type=str, required=True, help=MSG_NO_START_DATE, location='args')
        self.reqparse.add_argument('to_date', type=str, required=True, help=MSG_NO_END_DATE, location='args')
    ...
    def get(self, link_id, group_by):
        ...
        try:
            ...
            return api_response_error(MSG_WRONG_DATE, 400)

        if from_date > (to_date - timedelta(hours=23)):
            return api_response_error(MSG_STRT_DATE_GRTR_THAN_END, 400)

        if to_date > (datetime.now() + timedelta(hours=23)):
            return api_response_error(MSG_END_DATE_GRTR_THAN_TODAY, 400)

        if not self.validate_group_by(group_by):
            return api_response_error(MSG_WRONG_GRP_BY, 400)
        ...
        if (from_date + timedelta(hours=23)) < url_created:
            return api_response_error(MSG_LINK_WAS_CREATED_LATER, 400)
    ...
    def build_report(self, dates, redirects, group_by):
        ...
        for redirect in redirects:
            for date in dates:
                if date < redirect["datetime"] < (self.add_timedelta(date, group_by)):
